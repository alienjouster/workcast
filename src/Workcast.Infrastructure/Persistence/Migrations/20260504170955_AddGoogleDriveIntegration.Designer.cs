// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using Workcast.Infrastructure.Persistence;

#nullable disable

namespace Workcast.Infrastructure.Persistence.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260504170955_AddGoogleDriveIntegration")]
    partial class AddGoogleDriveIntegration
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Workcast.Core.Entities.AdScoring", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<Guid>("JobAdId")
                        .HasColumnType("uuid")
                        .HasColumnName("job_ad_id");

                    b.Property<double>("OverallScore")
                        .HasColumnType("double precision")
                        .HasColumnName("overall_score");

                    b.Property<string>("Recommendation")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Requirements")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("requirements");

                    b.Property<DateTimeOffset>("ScoredAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("scored_at");

                    b.Property<string>("Summary")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("summary");

                    b.HasKey("Id");

                    b.HasIndex("JobAdId")
                        .IsUnique()
                        .HasDatabaseName("ix_ad_scorings_job_ad_id");

                    b.ToTable("ad_scorings", (string)null);
                });

            modelBuilder.Entity("Workcast.Core.Entities.AppSettings", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer");

                    b.Property<int>("BoardAnalyzerMaxTokens")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(4096)
                        .HasColumnName("board_analyzer_max_tokens");

                    b.Property<string>("BoardAnalyzerModel")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("board_analyzer_model");

                    b.Property<string>("GoogleDriveBaseFolderId")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("google_drive_base_folder_id");

                    b.Property<string>("GoogleDriveBasePath")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasDefaultValue("jobs")
                        .HasColumnName("google_drive_base_path");

                    b.Property<string>("GoogleDriveRefreshToken")
                        .HasColumnType("text")
                        .HasColumnName("google_drive_refresh_token");

                    b.Property<int>("InterviewAnswerEvaluationMaxTokens")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(1024)
                        .HasColumnName("interview_answer_evaluation_max_tokens");

                    b.Property<string>("InterviewAnswerEvaluationModel")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("interview_answer_evaluation_model");

                    b.Property<int>("InterviewTrainerMaxTokens")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(4096)
                        .HasColumnName("interview_trainer_max_tokens");

                    b.Property<string>("InterviewTrainerModel")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("interview_trainer_model");

                    b.Property<int>("LetterGenerationMaxTokens")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(2048)
                        .HasColumnName("letter_generation_max_tokens");

                    b.Property<string>("LetterGenerationModel")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("letter_generation_model");

                    b.Property<byte[]>("ResumeContent")
                        .HasColumnType("bytea")
                        .HasColumnName("resume_content");

                    b.Property<string>("ResumeContentType")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("resume_content_type");

                    b.Property<string>("ResumeFileName")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("resume_file_name");

                    b.Property<int>("ResumeGenerationMaxTokens")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(8192)
                        .HasColumnName("resume_generation_max_tokens");

                    b.Property<string>("ResumeGenerationModel")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("resume_generation_model");

                    b.Property<string>("ResumeTemplateContent")
                        .HasColumnType("text")
                        .HasColumnName("resume_template_content");

                    b.Property<string>("ResumeTemplateFileName")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("resume_template_file_name");

                    b.Property<DateTimeOffset?>("ResumeTemplateUploadedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("resume_template_uploaded_at");

                    b.Property<DateTimeOffset?>("ResumeUploadedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("resume_uploaded_at");

                    b.Property<int>("ScoringMaxTokens")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(4096)
                        .HasColumnName("scoring_max_tokens");

                    b.Property<string>("ScoringModel")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("scoring_model");

                    b.HasKey("Id");

                    b.ToTable("app_settings", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            BoardAnalyzerMaxTokens = 4096,
                            BoardAnalyzerModel = "claude-sonnet-4-5",
                            GoogleDriveBasePath = "jobs",
                            InterviewAnswerEvaluationMaxTokens = 1024,
                            InterviewAnswerEvaluationModel = "claude-sonnet-4-5",
                            InterviewTrainerMaxTokens = 4096,
                            InterviewTrainerModel = "claude-sonnet-4-5",
                            LetterGenerationMaxTokens = 2048,
                            LetterGenerationModel = "claude-sonnet-4-6",
                            ResumeGenerationMaxTokens = 16384,
                            ResumeGenerationModel = "claude-sonnet-4-6",
                            ScoringMaxTokens = 4096,
                            ScoringModel = "claude-haiku-4-5-20251001"
                        });
                });

            modelBuilder.Entity("Workcast.Core.Entities.Application", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<string>("Company")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("company");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_at");

                    b.Property<string>("Description")
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<string>("ExternalId")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("external_id");

                    b.Property<string>("GoogleDriveFolderId")
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("google_drive_folder_id");

                    b.Property<bool>("IsInterviewDrillPending")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_interview_drill_pending");

                    b.Property<bool>("IsLetterGenerationPending")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_letter_generation_pending");

                    b.Property<bool>("IsResumeGenerationPending")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsScoringPending")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_scoring_pending");

                    b.Property<bool>("IsTrashed")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_trashed");

                    b.Property<string>("JobAdContent")
                        .HasColumnType("text")
                        .HasColumnName("job_ad_content");

                    b.Property<Guid?>("JobAdId")
                        .HasColumnType("uuid")
                        .HasColumnName("job_ad_id");

                    b.Property<string>("LastInterviewDrillError")
                        .HasColumnType("text")
                        .HasColumnName("last_interview_drill_error");

                    b.Property<string>("LastLetterGenerationError")
                        .HasColumnType("text")
                        .HasColumnName("last_letter_generation_error");

                    b.Property<string>("LastResumeGenerationError")
                        .HasColumnType("text");

                    b.Property<string>("LastScoringError")
                        .HasColumnType("text")
                        .HasColumnName("last_scoring_error");

                    b.Property<string>("Location")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("location");

                    b.Property<double?>("OverallScore")
                        .HasColumnType("double precision")
                        .HasColumnName("overall_score");

                    b.Property<DateTimeOffset?>("PostedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("posted_at");

                    b.Property<string>("Recommendation")
                        .HasColumnType("text")
                        .HasColumnName("recommendation");

                    b.Property<string>("Requirements")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("requirements");

                    b.Property<string>("SalaryRaw")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("salary_raw");

                    b.Property<DateTimeOffset?>("ScoredAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("scored_at");

                    b.Property<DateTimeOffset>("ScrapedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("scraped_at");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("text")
                        .HasDefaultValue("ToApply")
                        .HasColumnName("status");

                    b.Property<string>("StatusHistory")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("status_history");

                    b.Property<string>("Summary")
                        .HasColumnType("text")
                        .HasColumnName("summary");

                    b.Property<string>("Title")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("title");

                    b.Property<string>("Url")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasColumnName("url");

                    b.HasKey("Id");

                    b.HasIndex("CreatedAt")
                        .IsDescending()
                        .HasDatabaseName("ix_applications_created_at_desc");

                    b.HasIndex("IsTrashed")
                        .HasDatabaseName("ix_applications_is_trashed");

                    b.HasIndex("JobAdId")
                        .HasDatabaseName("ix_applications_job_ad_id");

                    b.ToTable("applications", (string)null);
                });

            modelBuilder.Entity("Workcast.Core.Entities.GeneratedLetter", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<Guid>("ApplicationId")
                        .HasColumnType("uuid")
                        .HasColumnName("application_id");

                    b.Property<DateTimeOffset>("GeneratedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("generated_at");

                    b.Property<string>("HtmlContent")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("html_content");

                    b.Property<bool>("IsManualEdit")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_manual_edit");

                    b.Property<string>("ModelUsed")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("model_used");

                    b.Property<int>("VersionNumber")
                        .HasColumnType("integer")
                        .HasColumnName("version_number");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationId")
                        .HasDatabaseName("ix_generated_letters_application_id");

                    b.HasIndex("ApplicationId", "VersionNumber")
                        .IsUnique()
                        .HasDatabaseName("ix_generated_letters_application_id_version_number");

                    b.ToTable("generated_letters", (string)null);
                });

            modelBuilder.Entity("Workcast.Core.Entities.GeneratedResume", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<Guid>("ApplicationId")
                        .HasColumnType("uuid")
                        .HasColumnName("application_id");

                    b.Property<DateTimeOffset>("GeneratedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("generated_at");

                    b.Property<string>("HtmlContent")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("html_content");

                    b.Property<bool>("IsManualEdit")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_manual_edit");

                    b.Property<string>("ModelUsed")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("model_used");

                    b.Property<string>("OptimizationLevel")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("optimization_level");

                    b.Property<int>("VersionNumber")
                        .HasColumnType("integer")
                        .HasColumnName("version_number");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationId")
                        .HasDatabaseName("ix_generated_resumes_application_id");

                    b.HasIndex("ApplicationId", "GeneratedAt")
                        .IsDescending(false, true)
                        .HasDatabaseName("ix_generated_resumes_application_id_generated_at_desc");

                    b.HasIndex("ApplicationId", "VersionNumber")
                        .IsUnique()
                        .HasDatabaseName("ix_generated_resumes_application_id_version_number");

                    b.ToTable("generated_resumes", (string)null);
                });

            modelBuilder.Entity("Workcast.Core.Entities.InterviewDrillPlan", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<Guid>("ApplicationId")
                        .HasColumnType("uuid")
                        .HasColumnName("application_id");

                    b.Property<DateTimeOffset>("GeneratedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("generated_at");

                    b.Property<string>("ModelUsed")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("model_used");

                    b.Property<string>("Questions")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("questions");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationId")
                        .IsUnique()
                        .HasDatabaseName("ix_interview_drill_plans_application_id");

                    b.ToTable("interview_drill_plans", (string)null);
                });

            modelBuilder.Entity("Workcast.Core.Entities.InterviewStep", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<Guid>("ApplicationId")
                        .HasColumnType("uuid")
                        .HasColumnName("application_id");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_at");

                    b.Property<DateOnly?>("Date")
                        .HasColumnType("date")
                        .HasColumnName("date");

                    b.Property<int?>("DurationMinutes")
                        .HasColumnType("integer")
                        .HasColumnName("duration_minutes");

                    b.Property<string>("Interviewers")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("interviewers");

                    b.Property<bool>("IsOnSite")
                        .HasColumnType("boolean")
                        .HasColumnName("is_on_site");

                    b.Property<string>("Notes")
                        .HasColumnType("text")
                        .HasColumnName("notes");

                    b.Property<string>("RemoteCallLink")
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasColumnName("remote_call_link");

                    b.Property<int>("StepNumber")
                        .HasColumnType("integer")
                        .HasColumnName("step_number");

                    b.Property<TimeOnly?>("Time")
                        .HasColumnType("time")
                        .HasColumnName("time");

                    b.Property<string>("Timezone")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasDefaultValue("CEST")
                        .HasColumnName("timezone");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationId")
                        .HasDatabaseName("ix_interview_steps_application_id");

                    b.ToTable("interview_steps", (string)null);
                });

            modelBuilder.Entity("Workcast.Core.Entities.JobAd", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<string>("Company")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("company");

                    b.Property<string>("Description")
                        .HasColumnType("text")
                        .HasColumnName("description");

                    b.Property<string>("ExternalId")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("external_id");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("is_active");

                    b.Property<bool>("IsManual")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_manual");

                    b.Property<bool>("IsPinned")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_pinned");

                    b.Property<bool>("IsRead")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("is_read");

                    b.Property<bool>("IsScoringPending")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsTrashed")
                        .HasColumnType("boolean");

                    b.Property<Guid?>("JobBoardId")
                        .HasColumnType("uuid")
                        .HasColumnName("job_board_id");

                    b.Property<string>("LastScoringError")
                        .HasColumnType("text");

                    b.Property<string>("Location")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("location");

                    b.Property<DateTimeOffset?>("PostedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("posted_at");

                    b.Property<string>("SalaryRaw")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("salary_raw");

                    b.Property<Guid?>("ScrapeRunId")
                        .HasColumnType("uuid")
                        .HasColumnName("scrape_run_id");

                    b.Property<DateTimeOffset>("ScrapedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("scraped_at");

                    b.Property<string>("Title")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("title");

                    b.Property<string>("Url")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasColumnName("url");

                    b.HasKey("Id");

                    b.HasIndex("ScrapeRunId");

                    b.HasIndex("ScrapedAt")
                        .IsDescending()
                        .HasDatabaseName("ix_job_ads_scraped_at_desc");

                    b.HasIndex("JobBoardId", "ExternalId")
                        .IsUnique()
                        .HasDatabaseName("ix_job_ads_job_board_id_external_id")
                        .HasFilter("external_id IS NOT NULL");

                    b.HasIndex("JobBoardId", "IsActive")
                        .HasDatabaseName("ix_job_ads_job_board_id_is_active");

                    b.HasIndex("JobBoardId", "Url")
                        .IsUnique()
                        .HasDatabaseName("ix_job_ads_job_board_id_url");

                    b.ToTable("job_ads", (string)null);
                });

            modelBuilder.Entity("Workcast.Core.Entities.JobBoard", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("created_at");

                    b.Property<DateTimeOffset?>("LastScrapedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("last_scraped_at");

                    b.Property<string>("Name")
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)")
                        .HasColumnName("name");

                    b.Property<string>("ScheduleCron")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("schedule_cron");

                    b.Property<string>("ScraperConfig")
                        .HasColumnType("jsonb")
                        .HasColumnName("scraper_config");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("status");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("updated_at");

                    b.Property<string>("Url")
                        .IsRequired()
                        .HasMaxLength(2048)
                        .HasColumnType("character varying(2048)")
                        .HasColumnName("url");

                    b.HasKey("Id");

                    b.HasIndex("Status")
                        .HasDatabaseName("ix_job_boards_status");

                    b.ToTable("job_boards", (string)null);
                });

            modelBuilder.Entity("Workcast.Core.Entities.ScrapeRun", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid")
                        .HasColumnName("id")
                        .HasDefaultValueSql("gen_random_uuid()");

                    b.Property<int>("AdsFound")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("ads_found");

                    b.Property<int>("AdsNew")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("ads_new");

                    b.Property<string>("Errors")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("errors");

                    b.Property<DateTimeOffset?>("FinishedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("finished_at");

                    b.Property<string>("HangfireJobId")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("hangfire_job_id");

                    b.Property<Guid>("JobBoardId")
                        .HasColumnType("uuid")
                        .HasColumnName("job_board_id");

                    b.Property<int>("PagesScraped")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("pages_scraped");

                    b.Property<DateTimeOffset>("StartedAt")
                        .HasColumnType("timestamptz")
                        .HasColumnName("started_at");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("status");

                    b.Property<string>("TriggeredBy")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("triggered_by");

                    b.HasKey("Id");

                    b.HasIndex("HangfireJobId")
                        .HasDatabaseName("ix_scrape_runs_hangfire_job_id");

                    b.HasIndex("JobBoardId", "StartedAt")
                        .IsDescending(false, true)
                        .HasDatabaseName("ix_scrape_runs_job_board_id_started_at_desc");

                    b.ToTable("scrape_runs", (string)null);
                });

            modelBuilder.Entity("Workcast.Core.Entities.AdScoring", b =>
                {
                    b.HasOne("Workcast.Core.Entities.JobAd", "JobAd")
                        .WithMany()
                        .HasForeignKey("JobAdId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("JobAd");
                });

            modelBuilder.Entity("Workcast.Core.Entities.GeneratedLetter", b =>
                {
                    b.HasOne("Workcast.Core.Entities.Application", "Application")
                        .WithMany()
                        .HasForeignKey("ApplicationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Application");
                });

            modelBuilder.Entity("Workcast.Core.Entities.GeneratedResume", b =>
                {
                    b.HasOne("Workcast.Core.Entities.Application", "Application")
                        .WithMany()
                        .HasForeignKey("ApplicationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Application");
                });

            modelBuilder.Entity("Workcast.Core.Entities.InterviewDrillPlan", b =>
                {
                    b.HasOne("Workcast.Core.Entities.Application", null)
                        .WithMany()
                        .HasForeignKey("ApplicationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Workcast.Core.Entities.InterviewStep", b =>
                {
                    b.HasOne("Workcast.Core.Entities.Application", null)
                        .WithMany()
                        .HasForeignKey("ApplicationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Workcast.Core.Entities.JobAd", b =>
                {
                    b.HasOne("Workcast.Core.Entities.JobBoard", "JobBoard")
                        .WithMany("JobAds")
                        .HasForeignKey("JobBoardId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("Workcast.Core.Entities.ScrapeRun", "ScrapeRun")
                        .WithMany("JobAds")
                        .HasForeignKey("ScrapeRunId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("JobBoard");

                    b.Navigation("ScrapeRun");
                });

            modelBuilder.Entity("Workcast.Core.Entities.ScrapeRun", b =>
                {
                    b.HasOne("Workcast.Core.Entities.JobBoard", "JobBoard")
                        .WithMany("ScrapeRuns")
                        .HasForeignKey("JobBoardId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("JobBoard");
                });

            modelBuilder.Entity("Workcast.Core.Entities.JobBoard", b =>
                {
                    b.Navigation("JobAds");

                    b.Navigation("ScrapeRuns");
                });

            modelBuilder.Entity("Workcast.Core.Entities.ScrapeRun", b =>
                {
                    b.Navigation("JobAds");
                });
#pragma warning restore 612, 618
        }
    }
}
